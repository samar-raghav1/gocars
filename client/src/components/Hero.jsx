  ...
  return (
    ...
    gap-14 bg-light text-center'>
        <motion.h1 initial={{y:50,opacity:0}} animate={{y:0,opacity:1}} transition={{duration:0.8,delay:0.2}}
         className='
        text-4xl md:text-5xl font-semibold'>Luxury cars on Rent</motion.h1>
        <motion.form initial={{y:50,opacity:0,scale:0.95}} animate={{y:0,opacity:1,scale:1}} transition={{duration:0.6,delay:0.4}}
            ...
            gap-10 min-md:ml-8'>
             <div className='flex flex-col items-start gap-2'>
              <select value={pickupLocation} onChange={(e)=>setPickupLocation(e.target.value)}  required>
                ...
                <div className='flex flex-col items-start gap-2'>
                  ...
                  <input value={pickupDate} onChange={e=>setPickupDate(e.target.value)} type="date" name="" id="pick-up date" min={new Date().toISOString().split('T')[0]}
             ...
             <motion.button whileHover={{scale:1.05}} whileTap={{scale:0.95}}
              className='flex items-center justify-center gap-1 px-9 py-3
                ...
                rounded-full cursor-pointer' id='search-btn'>
        ...
        <motion.img initial={{x:400,opacity:0}}
